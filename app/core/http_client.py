...
def _timeout_seconds() -> float:
    """Resolve an upstream timeout budget in seconds."""
    value = getattr(settings, "proxy_timeout_seconds", None) or getattr(settings, "timeout_seconds", None) or 60
    try:
        seconds = float(value)
    except Exception:
        seconds = 60.0
...
def _httpx_timeout() -> httpx.Timeout:
    # Apply the same timeout to connect/read/write/pool.
...
def get_async_httpx_client() -> httpx.AsyncClient:
    """Return an AsyncClient safe under pytest function-scoped event loops."""
...
# Backward-compatible name used by app.main

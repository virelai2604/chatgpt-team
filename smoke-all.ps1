...
.DESCRIPTION
  One-file “all green” smoke test for your OpenAI-compatible relay (e.g., Cloudflare Pages/Workers proxy).
  Required checks (fail -> non‑zero exit): 
    - GET /v1/models
    - POST /v1/chat/completions
    - POST /v1/embeddings
    - CORS preflight (OPTIONS) for /v1/chat/completions
  Optional checks (opt-in via switches): 
    - /health (info)
    - TTS (audio/speech)  -> MP3 with Accept: audio/mpeg fallback
    - STT (audio/transcriptions) using the TTS output as input
    - Images (images/generations) -> save PNG
    - Files API (upload/list/download/delete) with a temp JSONL
    - Assistants v2 (create thread/run and poll)

.PARAMETER BaseUrl
  Base URL of your relay, e.g. https://chatgpt-team.pages.dev

.PARAMETER ApiKey
  Optional. If your relay expects client Authorization, pass a key here.
  If omitted, the script sends no Authorization header and relies on relay-side key injection (if configured).

.PARAMETER Org
  Optional. OpenAI-Organization (also sends x-openai-organization for compatibility).

.PARAMETER Project
  Optional. OpenAI-Project (also sends x-openai-project for compatibility).

.PARAMETER Model
  Chat model to use. Default: o4-mini (change if your relay uses different defaults).

.PARAMETER EmbeddingModel
  Embeddings model to use. Default: text-embedding-3-small.

.PARAMETER TtsModel
  TTS model. Default: gpt-4o-mini-tts.

.PARAMETER SttModel
  STT model. Default: gpt-4o-mini-transcribe.

.SWITCHES
  -IncludeHealth, -IncludeAudio, -IncludeImages, -IncludeFiles, -IncludeAssistants

.EXAMPLES
  # Minimum “required only” checks
  .\smoke-all.ps1 -BaseUrl https://chatgpt-team.pages.dev

  # Include optionals and provide client Authorization
  .\smoke-all.ps1 -BaseUrl https://chatgpt-team.pages.dev -ApiKey "sk-..." -Org "org-..." -Project "proj-..." `
                  -IncludeHealth -IncludeAudio -IncludeImages -IncludeFiles -IncludeAssistants

.NOTES
  Requires: PowerShell 7+. Exits with code 1 if any required step fails.
#>
...
if ($IncludeAudio) {
  ...
  # STT (only if TTS produced a file)
  if (Test-Path $ttsFile) {
    try {
      ...
      $client.DefaultRequestHeaders.Add('User-Agent','relay-smoke/1.0')
...
# ----- Optional: Images (generations, b64) -----
...
# ----- Optional: Files API (upload/list/download/delete) -----
if ($IncludeFiles) {
...
# ----- Optional: Assistants v2 (create -> thread -> message -> run -> poll) -----

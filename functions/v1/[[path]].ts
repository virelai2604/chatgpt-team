...
export const onRequest: PagesFunction<Env> = async ({ request, env }) => {
  ...
  // Copy through only the headers we need to preserve content negotiation and body semantics
  const forward = request.headers;
  for (const [k, v] of forward.entries()) {
  ...
  // ALWAYS set auth from env, never trust the client
  out.set("authorization", `Bearer ${env.OPENAI_KEY}`);

  // Optional org/project/beta
  ...
  // Never parse/rebuild multipart/form-data — just stream the original body through.
